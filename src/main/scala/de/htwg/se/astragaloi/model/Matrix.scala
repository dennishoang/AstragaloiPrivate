...
case class Matrix[T](cols: Vector[Vector[T]]):
    def this(size: Int, filling: Int) = this(Vector.tabulate(size, size) { (col, row) => filling })
    val size: Int = cols.size
    def cell(col: Int, row: Int): T = rows(col)(row) // z.B: rows(3)(1): Zugriff auf erste Zelle Spalte 3
    def col(col: Int) = rows(col)
